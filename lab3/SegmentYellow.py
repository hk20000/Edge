...
boundaries = [
    ...
    ([20, 100, 100], [30, 255, 255])] # For Yellow (in HSV space)
...
while True:
    try:
        ...
        for (lower, upper) in boundaries:
            ...
            # If it's for Yellow, we'll need to convert the image to HSV
            if (lower[0] < 30 and upper[0] > 30):  # Yellow segmentation (uses HSV space)
                hsv_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)  # Convert to HSV
                mask = cv2.inRange(hsv_frame, lower, upper)  # Mask in HSV space
            else:
                mask = cv2.inRange(frame, lower, upper)  # Mask in BGR space
                
            output.append(cv2.bitwise_and(frame, frame, mask = mask))  #Segmented frames are appended
        
        # Output is appeneded to be of size Pixels X 3 (for R, G, B, Y)
        red_img = normalizeImg(output[0])
        green_img = normalizeImg(output[1])
        blue_img = normalizeImg(output[2])
        yellow_img = normalizeImg(output[3])  # For yellow segmentation
        
        # horizontal Concatination for displaying the images and colour segmentations
        catImg = cv2.hconcat([frame, red_img, green_img, blue_img, yellow_img])  # Added yellow_img to the concat
